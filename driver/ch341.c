/*
 * USB serial driver for USB to UART chip ch340, ch341, etc.
 *
...
static int ch341_control_out(struct ch341 *ch341, u8 request,
                u16 value, u16 index)
{	
    int retval;
    ...
    retval = usb_control_msg(ch341->dev, usb_sndctrlpipe(ch341->dev, 0),
        request, USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_DIR_OUT,
        value, index, NULL, 0, DEFAULT_TIMEOUT);

    dev_vdbg(&ch341->data->dev,
...
static int ch341_control_in(struct ch341 *ch341, 
...
static inline int ch341_set_control(struct ch341 *ch341, int control)
{
    ...
    return ch341_control_out(ch341, CMD_C2,
            value, 0x0000);
}
...
/*
 * Finish write. Caller must hold ch341->write_lock
 */
...
static void ch341_update_status(struct ch341 *ch341,
...
/*
 * Interrupt handlers for various CH341 device responses
 */
...
static int ch341_submit_read_urb(struct ch341 *ch341, int index, gfp_t mem_flags)
{
	...
	dev_vdbg(&ch341->data->dev, "%s - urb %d\n", __func__, index);
...
static void ch341_read_bulk_callback(struct urb *urb)
{
	...
	dev_vdbg(&ch341->data->dev, "%s - urb %d, len %d\n", __func__,
...
static void ch341_write_bulk(struct urb *urb)
{
	...
	dev_vdbg(&ch341->data->dev, "%s, len %d\n", __func__, urb->actual_length);
					...
					const unsigned char *buf, int count)
{
	...
	dev_vdbg(&ch341->data->dev, "%s - count %d\n", __func__, count);
...
static int ch341_tty_break_ctl(struct tty_struct *tty, int state)
{
	...
	retval = ch341_control_out(ch341, CMD_W, regval,
			reg_contents);
	if (retval < 0)
		dev_err(&ch341->data->dev, "%s - USB control write error (%d)\n",
				__func__, retval);
out:
	kfree(regbuf);
...
static int ch341_tty_tiocmget(struct tty_struct *tty)
{
	...
	spin_unlock_irqrestore(&ch341->read_lock, flags);
	dev_dbg(&ch341->data->dev, "%s - result = %x\n", __func__, result);

	return result;
}

static int ch341_tty_tiocmset(struct tty_struct *tty,
...
static int get_serial_info(struct ch341 *ch341, struct serial_struct __user *info)
{
	...
	tmp.closing_wait = ch341->port.closing_wait == ASYNC_CLOSING_WAIT_NONE ?
...
static int ch341_tty_ioctl(struct tty_struct *tty,
...
static int ch341_get(unsigned int baval,
						...
						struct ktermios *termios_old)
{
	...
	if (memcmp(&ch341->line, &newline, sizeof newline)) {
		...
		dev_dbg(&ch341->data->dev, "%s - set line: %d %d %d %d\n",
			__func__,
			newline.dwDTERate,
...
static void ch341_read_buffers_free(struct ch341 *ch341)
{
	...
	for (i = 0; i < ch341->rx_buflimit; i++)
		usb_free_coherent(usb_dev, ch341->readsize,
			  ch341->read_buffers[i].base, ch341->read_buffers[i].dma);
}
...
static int ch341_write_buffers_alloc(struct ch341 *ch341)
{
	...
	for (wb = &ch341->wb[0], i = 0; i < CH341_NW; i++, wb++) {
		wb->buf = usb_alloc_coherent(ch341->dev, ch341->writesize, GFP_KERNEL,
		    &wb->dmah);
		if (!wb->buf) {
			while (i != 0) {
				--i;
				--wb;
				usb_free_coherent(ch341->dev, ch341->writesize,
				    wb->buf, wb->dmah);
			}
			return -ENOMEM;
		}
		     ...
		     const struct usb_device_id *id)
{
	...
	for (i = 0; i < num_rx_buf; i++) {
		...
		rb->base = usb_alloc_coherent(ch341->dev, readsize, GFP_KERNEL,
								&rb->dma);
		if (!rb->base)
			goto alloc_fail6;
		rb->index = i;
		...
		usb_fill_bulk_urb(urb, ch341->dev,
			ch341->rx_endpoint,
			rb->base,
			ch341->readsize,
	...
	usb_fill_int_urb(ch341->ctrlurb, usb_dev,
			 ...
			 ch341->ctrl_buffer, ctrlsize, ch341_ctrl_irq, ch341,
	...
	tty_dev = tty_port_register_device(&ch341->port, ch341_tty_driver, minor,
			&data_interface->dev);
	if (IS_ERR(tty_dev)) {
		rv = PTR_ERR(tty_dev);
		goto alloc_fail7;
...
static int __init ch341_init(void)
{
	...
	ch341_tty_driver->init_termios.c_cflag = B9600 | CS8 | CREAD |
								HUPCL | CLOCAL;
